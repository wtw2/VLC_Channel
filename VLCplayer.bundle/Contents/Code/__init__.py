...
RE_IP_MAP    = Regex('^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$')
RE_PORT_MAP  = Regex('^[1-9][0-9]{0,4}$')
RE_PAGE_MAP  = Regex('^(/[0-9a-zA-Z_\-]*)*(?(1)/)(?:[0-9a-zA-Z_\-\.]+\.[0-9a-zA-Z]{2,4})?$')
RE_URL_MAP   = Regex('^http://(?:[0-9]{1,3}\.){3}[0-9]{1,3}:[1-9][0-9]{0,4}(?:/[0-9a-zA-Z_\-]*)*(?:/[0-9a-zA-Z_\-\.]+\.[0-9a-zA-Z]{2,4})?$')
# http://regex101.com/
...
#		return MessageContainer("Error", "That is not a valid IP address.")
	...
	url_vlc = 'http://%s:%s/%s' % (Prefs['vlc_host'], Prefs['vlc_port'], Prefs['vlc_page']) # dynamic
...
# Not using the URL service is not working yet
